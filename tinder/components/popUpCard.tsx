...
const PopUp = ({current ,displayed , setDisplayed , user }:PopUpPropsExtended)=>{
  return(
    <motion.div key="details-panel"
      ...
      <div className="scrollbarhidden flex text-lg flex-col gap-5 w-full  rounded-3xl scrollbar overflow-y-auto">
          ...
          {current.profile.bio && <div className="  glass   px-6 max-md:px-5 py-3">
            <p className=" font-semibold text-xl">About Me</p>
            <h1 className=" mt-2 text-sm ">{current.profile.bio}</h1>
          </div>}

          {current.profile.lookingFor &&<div className=" glass  px-6 max-md:px-5 py-3">
            <p>Looking for</p>
            <h1 className=" text-base ">{current.profile.lookingFor} </h1>
          </div>}

            {current.profile.livingIn &&<div className=" glass  px-6 max-md:px-5 py-3">
          ...
          {current.profile.age &&<div className=" glass  px-6 max-md:px-5 py-3">
            <p>Age</p>
            <h1 className=" text-base ">{current.profile.age}</h1>
          </div>}

          {current.profile.batch &&<div className=" glass  px-6 max-md:px-5 py-3">
            <p  className=" center gap-2 mb-2 !justify-start"><LuUniversity size={21} />Batch</p>
            <h1 className=" text-base ">{current.profile.batch}</h1>
          </div>}

          {current.profile.job &&<div className=" glass  px-6 max-md:px-5 py-3">
          ...
          {current.profile.height && current.profile.height >= 18 &&<div className=" glass  px-6 max-md:px-5 py-3">
            <p >Height</p>
            <h1 className=" text-base ">{current.profile.height}</h1>
          </div>}
         {current.profile.languages && <div className=" glass  px-6 max-md:px-5 py-3">
          ...
          <div className=" glass  px-6 max-md:px-5 py-3">
