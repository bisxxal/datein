...
const PopUp = ({ current, displayed, setDisplayed, user }: PopUpPropsExtended) => {
  return (
    <div
      className={` popupcardAnimation text-white h-[75vh] shadow-xl absolute z-[20] border -t border-white/70 bg-[#0000002e] py-3 backdrop-blur-[20px] justify-between w-[450px] max-md:w-[95%] mt-14 px-5 bottom-0 pt-[5px] rounded-3xl flex`}>
      ...
      <div className="  flex text-lg flex-col gap-5 w-full  rounded-3xl scrollbar overflow-y-auto">
        ...
        {current.profile.bio && <div className="  glass4 px-3 max-md:px-4  py-2">
          <p className=" font-semibold text-xl">About Me</p>
          <h1 className=" mt-2 text-sm ">{current.profile.bio}</h1>
        </div>}

        {current.profile.lookingFor && <div className=" glass4 px-3 max-md:px-4  py-2">
          <p>Looking for</p>
          <h1 className=" text-base ">{current.profile.lookingFor} </h1>
        </div>}

        {current.profile.livingIn && <div className=" glass4  px-3 max-md:px-3 py-3">
        ...
        {current.profile.age && <div className=" glass4 max-md:px-4 px-3 py-2">
          <p>Age</p>
          <h1 className=" text-base ">{current.profile.age}</h1>
        </div>
        }

        {current.profile.batch && <div className=" glass4 max-md:px-4 px-3 py-2">
          <p className=" center gap-2 mb-2 !justify-start"><LuUniversity size={21} />Batch</p>
          <h1 className=" text-base ">{current.profile.batch}</h1>
        </div>}

        {current.profile.job && <div className=" glass4 max-md:px-4 px-3 py-2">
        ...
        {current.profile.height && current.profile.height >= 18 && <div className=" glass4 max-md:px-4 px-3 py-2">
          <p >Height</p>
          <h1 className=" text-base ">{current.profile.height}</h1>
        </div>}
        {current.profile.languages && <div className=" glass4 max-md:px-4 px-3 py-2">
        ...
        <div className=" glass4 max-md:px-4 px-3 py-2">
